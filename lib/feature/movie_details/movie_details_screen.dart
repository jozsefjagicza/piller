...
class _MovieDetailsScreenState extends State<MovieDetailsScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final movieProvider = locator<MovieDetailsProvider>();
      String languageCode =
          "${Localizations.localeOf(context).languageCode}-${Localizations.localeOf(context).countryCode}";
      movieProvider.getMovieDetails(widget.movie.id, languageCode);
      _checkFavorite();
    });
  }
      ...
      child: Consumer<MovieDetailsProvider>(
        builder: (context, movieProvider, _) {
          ...
          if (movieProvider.errorMessage != null) {
            return Center(
              child: Text(
                tr('general_error', args: [movieProvider.errorMessage!]),
                style: AppTextStyles.itemTitle,
              ),
            );
          }

          final movie = movieProvider.movieDetails;
          if (movie == null) {
